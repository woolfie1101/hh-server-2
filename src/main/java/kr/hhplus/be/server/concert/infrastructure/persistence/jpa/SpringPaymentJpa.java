...
public interface SpringPaymentJpa extends JpaRepository<PaymentEntity, UUID> {
    ...
    List<PaymentEntity> findBySeatId(UUID seatId);
    ...
    @Query("SELECT COALESCE(SUM(p.amount), 0) FROM PaymentEntity p WHERE p.userId = :userId AND p.status = 'SUCCESS'")
    BigDecimal calculateTotalAmountByUserId(@Param("userId") UUID userId);
} 